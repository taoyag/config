...
"  Last Modified: 2009-12-19 19:00
"        Version: 2.01
"=============================================================================
...
if !exists('MSWindows')
  let MSWindows = has('win95') + has('win16') + has('win32') + has('win64')
endif
...
"howmファイルの拡張子
let g:QFixHowm_FileExt = substitute(g:howm_filename,'.*\.','','')
"howmファイルタイプ
...
"常にここを起点に検索する
if !exists('g:QFixHowm_SearchDir')
  let g:QFixHowm_SearchDir = ''
endif
"最近更新ファイル検索日数
...
if !exists('g:QFixHowm_ReminderHolidayName')
  let g:QFixHowm_ReminderHolidayName = '元日\|成人の日\|建国記念の日\|昭和の日\|憲法記念日\|みどりの日\|こどもの日\|海の日\|敬老の日\|体育の日\|文化の日\|勤労感謝の日\|天皇誕生日\|春分の日\|秋分の日\|振替休日\|国民の休日'
endif
...
let s:sch_dow      = '\c\(Sun\|Mon\|Tue\|Wed\|Thu\|Fri\|Sat\)'
let s:sch_ext      = '-@!+~.'
...
if g:QFixHowm_Default_Key > 0
  ...
  exec 'silent! nnoremap <silent> '.s:QFixHowm_Key.'b  :BGrep<CR>'
...
augroup QFixHowm
  autocmd!
  autocmd VimLeave * if s:QFixHowm_Init | call QFixHowmSaveMru(2) | endif
  exec "autocmd BufRead      *.".g:QFixHowm_FileExt." call QFixHowmSaveMru(1)"
  exec "autocmd BufLeave     *.".g:QFixHowm_FileExt." call QFixHowmSaveMru(0)"
  exec "autocmd BufWinLeave  *.".g:QFixHowm_FileExt." call QFixHowmSaveMru(0)"
  exec "autocmd VimLeavePre  *.".g:QFixHowm_FileExt." call QFixHowmSaveMru(0)"
  exec "autocmd BufReadPre   *.".g:QFixHowm_FileExt." silent! exec 'call QFixHowmInit()'"
  exec "autocmd BufReadPost  *.".g:QFixHowm_FileExt." if g:QFixHowm_ForceEncoding |exec 'edit! ++enc='.g:howm_fileencoding.' ++ff='.g:howm_fileformat |endif"
"  exec "autocmd BufEnter     *.".g:QFixHowm_FileExt." nmap <silent> <buffer> <expr> <CR> QFixHowmActionLock()"
  exec "autocmd BufRead,BufNewFile *.".g:QFixHowm_FileExt." exec 'setlocal filetype='.g:QFixHowm_FileType.'| call QFixHowmHighlight()'"
  exec "autocmd BufWritePre  *.".g:QFixHowm_FileExt." call QFixHowmInsertLastModified()"
  exec "autocmd BufWritePost *.".g:QFixHowm_FileExt." call QFixHowmBufWritePost()"
  ...
  autocmd BufWinLeave quickfix call QFixHowmSaveMru(-1)
augroup END
...
"  vnoremap <buffer> <silent> <S-CR>  :call QFixHowmCmd_CR(1)<CR>
  ...
  vnoremap <buffer> <silent> &  :call QFixHowmCmd_AT('user')<CR><ESC>:call QFixHowmUserCmd(g:QFixHowm_MergeList)<CR>
...
function! QFixHowmListAllTitle(pattern, days, ...)
  ...
  let l:QFixHowm_SearchDir = g:QFixHowm_SearchDir
  let g:QFixHowm_SearchDir = ''
  call QFixHowmListAll(pattern, a:days)
  let g:QFixHowm_SearchDir = l:QFixHowm_SearchDir
endfunction
...
"    set noautochdir
  ...
  if g:QFixHowm_SearchDir != ''
    let g:howm_dir = g:QFixHowm_SearchDir
  endif
  ...
  call MyGrep(gpattern, g:howm_dir, g:QFixHowm_SearchHowmFile, g:howm_fileencoding, addflag)
  let wlist = []
  let clist = []
...
"    let g:howm_dir = g:QFixHowm_SearchDir
  ...
  if !exists('g:mygrepprg') || g:mygrepprg == 'internal' || g:mygrepprg == ''
    ...
    let searchWord = escape(searchWord, '/')
  else
    if g:QFixHowm_RecentMode == 3 || g:QFixHowm_RecentMode == 4
...
function! QFixHowmListRecentReplaceTitle(file, pattern, lnum)
  ...
  if s:prevfname != a:file
    silent! %delete _
"    if g:howm_fileencoding != &enc || g:QFixHowm_ForceEncoding == 0
    if 1
      let tmpfile = a:file
      ...
      let retval = readfile(expand(a:file))
...
"    echo flist[n] llist[n]
    ...
    if s == -1
      let rez = rez . g:QFixHowm_MergeEntrySeparator.g:howm_glink_pattern.' '.g:howm_dir.'/'. flist[n] . "\<NL>"
      let rez = rez . entry
    endif
  endfor
  silent! exec 'silent! e '.s:howmtempfile
...
function! QFixHowmATCombine(file, lnum)
  ...
  if g:QFixHowm_ForceEncoding
    silent! exec '0read ++enc='.g:howm_fileencoding.' ++ff='.g:howm_fileformat.' '.a:file
  else
    silent! exec '0read '.a:file
  endif
  silent! $delete _
  let lnum = a:lnum
  let l:QFixHowm_Title = escape(g:QFixHowm_Title, g:QFixHowm_EscapeTitle)
  ...
  let list = {'lnum':lnum - start+1, 'text':getline(start), 'filename':g:howm_dir.'/'.a:file, 'start':start, 'end':end}
...
function! QFixHowmAddKeyword()
  ...
  while 1
    ...
    let keyword = substitute(keyword, '\s*$', '', '')
    if count(s:KeywordDic, keyword) == 0
...
function! QFixHowmRebuildKeyword()
  ...
  let l:howm_dir = g:howm_dir
  if g:QFixHowm_SearchDir != ''
    let l:howm_dir = g:QFixHowm_SearchDir
  endif
  let prevPath = getcwd()
  let prevPath = escape(prevPath, ' ')
  if exists('+autochdir')
...
function! QFixHowmHighlight()
  ...
  syntax clear actionlockKeyword
...
if !exists('g:QFixHowm_ScheduleSwActionLock')
  let g:QFixHowm_ScheduleSwActionLock= ['Sun)', 'Mon)', 'Tue)', 'Wed)', 'Thu)', 'Fri)', 'Sat)']
endif
...
"      let file = escape(file, '\')
      return ":exec 'call QFixEditFile(\"".file."\")'\<CR>"
      return "\<ESC>"
    endif
  ...
  call setpos('.', save_cursor)
  if exists('g:QFixHowm_UserSwActionLock2')
    let ret = QFixHowmSwitchActionLock(g:QFixHowm_UserSwActionLock2)
    if ret != "\<CR>"
      return ret
    endif
  endif
  call setpos('.', save_cursor)
  if exists('g:QFixHowm_UserSwActionLock3')
    let ret = QFixHowmSwitchActionLock(g:QFixHowm_UserSwActionLock3)
    if ret != "\<CR>"
      return ret
    endif
  endif
...
function! s:OpenUri(uri)
  let cmd = ''
  if exists('g:QFixHowm_OpenURIcmd') && g:QFixHowm_OpenURIcmd != ''
    let cmd = g:QFixHowm_OpenURIcmd
    let cmd = substitute(cmd, '%s', escape(a:uri, '&'), '')
    let cmd = escape(cmd, '%#')
    silent exec cmd
    return 1
  endif
...
"    return "\<CR>"
  ...
  exec "nmap <silent> " . s:QFixHowm_Key . g:QFixHowm_MacroActionKey . " " . ":<C-u>call QFixHowmSaveMru(1)<CR>:CloseQFixWin<CR>" .substitute(s:QFixHowm_MacroActionCmd, '^\s*', '', '')
...
function! QFixHowmOpenCursorline()
  ...
  let pos = match(strline, g:howm_glink_pattern)
  if strline =~ l:MyOpenVim_ExtReg && col('.') >= match(strline, g:howm_glink_pattern)
    return -1
  endif
  if pos > -1 && col('.') > pos
    let glinkuri = substitute(strline, '^.*'.g:howm_glink_pattern.'\s*', '', '')
    let mx = '^\(http\|ftp\|file\)://.*'
    let uri = matchstr(glinkuri, mx)
  ...
  let mx = '\(http\|ftp\|file\)://[-{}!#%&+,./[:alnum:]:;=?@_~\\]\+'
  let uri = matchstr(expand('<cWORD>'), mx)
  if uri.'x' == 'x'
    let mx = '\([A-Za-z]:[/\\]\|\~/\)[-!#%&+,./0-9:;=?@A-Za-z_~\\]\+'
    let uri = matchstr(expand('<cWORD>'), mx)
    let searchWord = '^'.s:sch_dateCmd
    let str = matchstr(getline('.'), searchWord)
    if match(getline('.'), searchWord) > -1 && strlen(str) >= prevcol
      return "\<CR>"
    endif
    if uri.'x' != 'x'
      if uri =~ l:MyOpenVim_ExtReg
        return -1
      else
        let ext = matchstr(uri, '[^.]\+$')
        if exists('g:QFixHowm_Opencmd_'.ext)
...
"  CloseQFixWin
  ...
  for d in s:MruDic
    ...
    let [min, max] = QFixHowmEntryRange(escape(expand(file), ' '), d['text'])
    if min == 0
      call remove(s:MruDic, mruidx)
      continue
    endif
...
function! QFixHowmSaveMru(write)
  ...
  let file = substitute(file, '/\+', '/', 'g')
  if file =~ g:QFixHowm_Menufile || file =~ g:QFixHowm_MRU_Ignore
    return
  endif
  let lnum = line('.')
  let text = getline('.')
...
function! QFixHowmHelp()
  silent! exec 'split QFixHowmHelp.vim'
  silent! exec 'setlocal filetype='.g:QFixHowm_FileType
  setlocal buftype=nofile
...
function! QFixHowmSaveAutolinkTag(keyword, file, lnum, mode)
  ...
  call writefile(sort(tdic), tfile)
endfunction
...
function! s:QFixHowmMakeScheduleList(sdic)
  ...
  if g:QFixHowm_ForceEncoding
    silent! exec '0read ++enc='.g:howm_fileencoding.' ++ff='.g:howm_fileformat.' '.file
  else
    silent! exec '0read '.file
  endif
  silent! $delete _
  call cursor(lnum,1)
...
function! s:QFixHowmParseScheduleList(sdic)
  for d in a:sdic
    let pattern = '.*'.s:sch_dateT.s:sch_Ext.'\d*\s*'.s:sch_dow.'\?\s*'
...
"ランダムウォークリスト作成
silent! function QFixHowmRandomList(result, len)
  let len   = a:len
  let range = len(a:result)
  if len >= range
    return a:result
  endif
  let result = []
